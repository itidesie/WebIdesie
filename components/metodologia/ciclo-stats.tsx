...
interface CicloStatsProps {
  stats: Stat[]
}

/**
 * Banda de cifras reales — mismas 4 que la versión anterior, pero sin
 * `#006cff` hardcodeado (usa los tokens de marca) y con entrada escalonada
 * en vez de estática.
 */
export function CicloStats({ stats }: CicloStatsProps) {
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-8 px-6 text-center sm:px-8 md:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.label} data-stat>
            <div className="ciclo-mono text-4xl font-black text-white md:text-5xl">{stat.value}</div>
            <div className="mt-2 text-sm font-medium text-white/90 md:text-base">{stat.label}</div>
            <div className="ciclo-eyebrow mt-1 text-white/65">{stat.note}</div>
          </div>
        ))}
      </div>
    </section>
